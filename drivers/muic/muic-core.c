...
#include "universal/muic-internal.h"
...
#if defined(CONFIG_MUIC_SUPPORT_EARJACK)
static void muic_earjack_intent_late(struct work_struct *work)
{
	muic_data_t *pmuic = 
			container_of(work, muic_data_t, earjack_detach_work.work);

	get_vps_data(pmuic, &pmuic->vps);

	pr_info("[MUIC] %s, adc:0x%x\n", __func__, pmuic->vps.s.adc);
	
	if (pmuic->vps.s.adc != 0x1E)
		switch_set_state(&switch_earjack, 0);

	return;
}

void muic_init_earjack_work(muic_data_t *pmuic)
{
	wpmuic = pmuic;

	INIT_DELAYED_WORK(&wpmuic->earjack_detach_work, muic_earjack_intent_late);
	pr_info("[MUIC] %s\n", __func__);
...
static int muic_earjack_intent(int state)
{
	...
	if (state == 1)
		...
		schedule_delayed_work(&wpmuic->earjack_detach_work, msecs_to_jiffies(300));
#endif
	return NOTIFY_OK;
}
			...
			unsigned long action, void *data)
{
	...
	if (attached_dev == ATTACHED_DEV_JIG_UART_ON_MUIC) {
...
#if defined(CONFIG_MUIC_SUPPORT_EARJACK) && defined(CONFIG_MUIC_UNIVERSAL_SM5703)
		if (action == MUIC_NOTIFY_CMD_DETACH) {
			switch (attached_dev) {
				...
				case ATTACHED_ERROR_EARJACK_MUIC:
	...
	case ATTACHED_DEV_VOLUP_MUIC:
	case ATTACHED_DEV_LP_VOLUP_MUIC:		
		return muic_earjackkey_intent(3);
		break;		
	case ATTACHED_DEV_VOLDN_MUIC:
	case ATTACHED_DEV_LP_VOLDN_MUIC:
		return muic_earjackkey_intent(4);
		break;
	case ATTACHED_STUCK_EARJACK_MUIC:
#else
	case ATTACHED_DEV_SEND_MUIC:
